...
# MAGIC new col with current timestamp
...
df_users.withColumn("curent_date",current_date()).display()
...
df_users.withColumnRenamed("id","emp_id").withColumn("current_date",current_date()).withColumn("full_name",concat("name","last_name")).drop("is_customer","name","last_name").display()
